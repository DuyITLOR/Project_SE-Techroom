...
export const giveFeedback = async (req, res) => {
    ...
    const newFeedback = await Feedback.addFeedback(classID,teacherID, studentID, tagID,text)
    if (!newFeedback) {
        return res.status(400).send({
        success: false,
...
export const updateTeacherFeedback = async (req, res) => {
    ...
    const updatedFeedback = await Feedback.updateFeedback(feedbackID, content, tagID)
    if (!updatedFeedback) {
        return res.status(400).send({
        success: false,
